...
def compute_p_r_f1(preds: List[Union[int, str]], golds: List[Union[int, str]], prefix=None, pos_label=1, average=None) -> dict:
    """
    Computes precision, recall, and f1 scores for predictions of classification tasks,
    both for each class and the overall scores.

    Args:
        preds (List[Union[int, str]]): List of predictions.
        golds (List[Union[int, str]]): List of ground truth labels.
        prefix (str, optional): Metric name prefix of results.
        pos_label (optional): The positive label, used in binary classification.
        average (str, optional): The type of averaging to be performed in multi-class tasks.
            Options are ['micro', 'macro', 'weighted', None].

    Returns:
        results (dict): A dictionary mapping metric names to their scores in percentage.
                        Includes precision, recall, and f1 for each class and overall.
    """
    if not isinstance(preds[0], type(golds[0])):
        logging.warning(f"The element types of golds ({type(golds[0])}) and predictions ({type(preds[0])}) "
                        f"is different, this will cause invalid evaluation results.")
    ...
    # Overall scores (based on the given average)
    if average:
        f1 = f1_score(y_true=golds, y_pred=preds, average=average)
        p = precision_score(y_true=golds, y_pred=preds, average=average)
        r = recall_score(y_true=golds, y_pred=preds, average=average)
    else:
        # For binary classification case, use pos_label
        f1 = f1_score(y_true=golds, y_pred=preds, pos_label=pos_label)
        p = precision_score(y_true=golds, y_pred=preds, pos_label=pos_label)
        r = recall_score(y_true=golds, y_pred=preds, pos_label=pos_label)

    # Adding the overall scores to the results dictionary
    results.update({
        f"{prefix}/precision" if prefix else "precision": p * 100,
        f"{prefix}/recall" if prefix else "recall": r * 100,
        f"{prefix}/f1" if prefix else "f1": f1 * 100
    ...
    per_class_precision = precision_score(y_true=golds, y_pred=preds, labels=unique_labels, average=None)
    per_class_recall = recall_score(y_true=golds, y_pred=preds, labels=unique_labels, average=None)
    per_class_f1 = f1_score(y_true=golds, y_pred=preds, labels=unique_labels, average=None)

    per_class_accuracy = []
    for label in unique_labels:
        label_preds = [1 if pred == label else 0 for pred in preds]
        label_golds = [1 if gold == label else 0 for gold in golds]
        per_class_accuracy.append(accuracy_score(label_golds, label_preds) * 100)
        
    for idx, label in enumerate(unique_labels):
        # Add each class's precision, recall, and F1 to the results
        results.update({
            f"{prefix}/class_{label}_acc" if prefix else f"class_{label}_accuracy": per_class_accuracy[idx],
            f"{prefix}/class_{label}_precision" if prefix else f"class_{label}_precision": per_class_precision[idx] * 100,
            f"{prefix}/class_{label}_recall" if prefix else f"class_{label}_recall": per_class_recall[idx] * 100,
            f"{prefix}/class_{label}_f1" if prefix else f"class_{label}_f1": per_class_f1[idx] * 100,
        })
